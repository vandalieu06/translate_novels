...
from novel_cli.core.models import LLMTranslate
...
class Translate:
    ...
    def translate_novel(self, chapter: str):
        total_words = len("".join(chapter).split())
        print(f"Length novel: {total_words} words")

        # Dividimos el texto en fragmentos
        chunks = self.split_text_by_words(
            chapter, max_words_per_chunk=LLMTranslate.max_words
        )
